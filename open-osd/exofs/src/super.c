 ...
 * SYSFS
 *****************************************************************************/
/** hs: userlevel application (anfs) creates collections for active operations.
 * This will be informed via sysfs to adjust the object id counter (s_nextid)
 * which is kept in sb.
 */
...
static struct exofs_attr exofs_attr_sync_id = {
	.attr.name = "sync_id",
...
static struct attribute * exofs_attrs[] = {
	&exofs_attr_sync_id.attr,
	NULL
};

static ssize_t exofs_attr_show(struct kobject *kobj, struct attribute *attr,
				char *buf)
{
...
static struct sysfs_ops exofs_attr_ops = {
	.show	= exofs_attr_show,
	.store	= exofs_attr_store,
};

static struct kobj_type exofs_ktype = {
	.sysfs_ops	= &exofs_attr_ops,
	.default_attrs	= exofs_attrs,
};
...
static int exofs_fill_super(struct super_block *sb, void *data, int silent)
{
	...
	printk(KERN_INFO "exofs s_id: %s, devname: %s\n",
			sb->s_id, opts->dev_name);
	...
	spin_lock_init(&sbi->s_nextid_lock);
	...
	/* register kobject */
	sbi->s_kobj.kset = exofs_kset;
	ret = kobject_init_and_add(&sbi->s_kobj, &exofs_ktype, NULL, "%s",
				strrchr(opts->dev_name, '/') + 1);
	if (ret) {
		if (!silent)
			EXOFS_ERR("ERROR: Failed to add kobject\n");
		goto free_sbi;
	}
